...
const Signup = () => {
  ...
  const onFinish = async (values) => {
    const { username, password, email, role, linkcv, age, phon_number } = values;

    try {
      const [usernameResponse, emailResponse] = await Promise.all([
        axios.get(`https://64890c550e2469c038fe9625.mockapi.io/VN_HS/user?userName=${username}`),
        axios.get(`https://64890c550e2469c038fe9625.mockapi.io/VN_HS/user?email=${email}`),
      ]);
    ...
    try {
      const response = await axios.post('https://64890c550e2469c038fe9625.mockapi.io/VN_HS/user', newUser);
